...
void ButtonISR(void) {
    while(1){
    Semaphore_pend(ScanButton, BIOS_WAIT_FOREVER);

    // read hardware button state
    uint32_t gpio_buttons =
            ~GPIOPinRead(GPIO_PORTJ_BASE, 0xff) & (/*GPIO_PIN_4 | GPIO_PIN_3 | GPIO_PIN_2 |*/ GPIO_PIN_1 | GPIO_PIN_0); // EK-TM4C1294XL buttons in positions 0 and 1
    ...
    char mail = (char)presses;

    if(presses == 4 || presses == 8 || presses == 16 || presses == 32){
        box = Mailbox_post(ButtonBox, &mail, 0);
    }
